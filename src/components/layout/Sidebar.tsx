...
import type { SidebarProps, View } from '../../types'
...
export default function Sidebar({ currentView, onNavigate, user, onLogout }: SidebarProps) {
  return (
    <aside className="w-64 flex-shrink-0 flex flex-col h-screen bg-slate-950 border-r border-slate-800/60 justify-between">
      <div>
        {/* Logo */}
      ...
      {/* 🔐 High-End Google Profile Footer Integration */}
      <div className="px-3 py-4 border-t border-slate-800/60 bg-slate-950">
        <div className="flex items-center gap-3 px-2 py-2.5 rounded-lg border border-transparent min-w-0">
          {user?.picture ? (
            <img 
              src={user.picture} 
              alt="Google Profile" 
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-slate-200 truncate">{user?.name || 'Operator Context'}</p>
            <p className="text-[10px] text-slate-500 truncate font-mono">{user?.email || 'session_active'}</p>
          </div>
        </div>
        ...
        <button
          onClick={onLogout}
          className="mt-2 w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-slate-900 hover:bg-red-950/40 border border-slate-800 hover:border-red-900/40 text-slate-400 hover:text-red-400 text-[10px] font-mono tracking-wide transition-all duration-150 cursor-pointer"
        >
